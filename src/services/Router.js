...
const RouterMap = {
  // '/': (match) => createElement('quiz-app'),
  '/': (match) => createElement('time-comp'),
  '/users': () => createElement('users-page'),
  '/projects': () => createElement('projects-page'),
  '/concepts': () => createElement('h1', 'Concepts Page'),
...
function showProject(match)
{
  ...
  const wrapper = createElement('project-page');
  wrapper.appendChild(createElement('quiz-question'));
  return wrapper;
}

const Router = {
  init() {
    this.routerSelector = '#root';
  ...
  renderPage(path) {
    ...
    const root = document.querySelector(this.routerSelector);
    if (!root) 
      throw new Error(`Missing router root element that '${this.routerSelector}' can select`);
    root.innerHTML = '';
    root.appendChild(pageContent || createElement('br'));
    window.scrollX = 0;
    window.scrollY = 0;
  },
